...
from development.elements import CTooltip, CLayout
from development.model import Instances

class Navbar(QFrame, Instances):
    def __init__(
        self,
        objectName: str = "Navbar",
        spacing: int = 5,
        width: int = None,
        height: int = None,
    ...
    def __layout__(self):
        layout = CLayout(self)
        self.main_layout = layout.horizontal(spacing=self._spacing)
    ...
    def add_to_layout(self, widgets: list[QWidget | QLayout]):
        for w in widgets:
            if isinstance(w, QWidget):
                self.main_layout.addWidget(w)
            elif isinstance(w, QLayout):
                self.main_layout.addLayout(w)
            else:
                self.main_layout.addItem(w)
